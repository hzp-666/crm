layui.use(['table', 'layer', "form"], function () {
    ...
    table.on('toolbar(customers)', function (obj) {
        ...
        switch (obj.event) {
            ...
            case "order":
                ...
                break;
            case "link":
                openLinkInfoDialog(checkStatus.data);
                break;
            case "recode":
                openContactInfoDialog(checkStatus.data);
                break;
        }
        ;
    });
    ...
    // 打开联系人查看页面
    function openLinkInfoDialog(datas) {
        if (datas.length == 0) {
            layer.msg("请选择待查看联系人对应客户!", {icon: 5});
            return;
        }
        if (datas.length > 1) {
            layer.msg("暂不支持批量查看!", {icon: 5});
        ...
        var url = ctx + "/customer/linkInfoPage?cid=" + datas[0].id;
        layui.layer.open({
            title: "客户管理-联系人信息展示",
            type: 2,
            area: ["700px", "500px"],
    ...
    // 打开交往记录查看页面
    function openContactInfoDialog(datas) {
        if (datas.length == 0) {
            layer.msg("请选择待查看交往记录对应客户!", {icon: 5});
            return;
        }
        if (datas.length > 1) {
            layer.msg("暂不支持批量查看!", {icon: 5});
        ...
        var url = ctx + "/customer/contactInfoPage?cid=" + datas[0].id;
        layui.layer.open({
            title: "客户管理-交往记录信息展示",
            type: 2,
            area: ["700px", "500px"],
