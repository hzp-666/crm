layui.use(['table', 'layer', "form"], function () {
    ...
    table.on("tool(customerLosses)", function (obj) {
        ...
        if (layEvent === "info") {
            openCustomerLossesInfoDialog(obj.data.id);
        } else if (layEvent === "add") {
            openAddCustomerLossesInfoDialog(obj.data.id);
        }
    });

    // 打开添加机会数据页面
    function openCustomerLossesInfoDialog(sid) {
        var url = ctx + "/customer_loss/customerLossesInfoPage?id=" + sid;
        var title = "流失管理管理-详情查看";
    ...
    function openAddCustomerLossesInfoDialog(sid) {
        var url = ctx + "/customer_loss/addCustomerLossesInfoPage?id=" + sid;
        var title = "流失管理管理-添加暂缓";
